...
function TestComponent({ initialData = {}, mutate, isPending }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit, onError)} className="mx-auto w-full bg-white p-8  md:max-w-5/6">
      <div>
        <h2 className="text-3xl font-bold text-gray-800 text-center pb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="relative pb-8">
          ...
          <select
            className="appearance-none w-full border-2 border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 p-3"
            {...register("gender")}
          >
            <option value="M">Чоловік</option>
            <option value="F">Жінка</option>
          </select>
          <IoIosArrowDropdownCircle className="w-7 h-7 pointer-events-none absolute top-11 right-2 text-gray-300" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="relative pb-8">
          ...
          <div className="flex items-center gap-4">
            <label
              htmlFor="imageUpload"
              className="cursor-pointer inline-flex items-center p-3 bg-slate-400 text-white text-sm font-semibold rounded-lg shadow-sm hover:bg-slate-600 transition"
            >
              Обрати файл
            </label>
      ...
      {isEditMode && (
        <div className="pb-6">
          ...
          <button
            type="button"
            className="cursor-pointer inline-flex items-center p-3 mb-2 bg-slate-400 text-white text-sm font-semibold rounded-lg shadow-sm hover:bg-slate-600 transition"
            onClick={() => {
              setValue("password", "");
          ...
          {wantChangePassword && (
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 space-y-4 transition-all">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                <PasswordInput
                  label="Пароль"
                  placeholder="Введіть пароль"
