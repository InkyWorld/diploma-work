...
from app.repository.shift_supervisor import get_shift_plan as get_sp, update_shift_plan as shift_plan_update, set_worker_on_event as swe, get_workers, get_all_events_done
from app.models.enums import ShiftEnum 
from app.db.redis import get_redis
from app.db.database import get_db
from app.schemas.shift_supervisor import UserResponseSchema, EventSchema
...
@shift_supervisor_router.post("/update_shift_plan", response_model=List[AircraftDataResponseSchema], dependencies=[Depends(shift_supervisor_only_access)], status_code=status.HTTP_200_OK)
...
async def set_worker_on_event(worker_ids: list[int], shift: ShiftEnum, event_performance_number_identifier: int, redis: Redis = Depends(get_redis), db: AsyncSession = Depends(get_db)):
    try:
        users = await get_users_by_ids(worker_ids, db)
        for user in users:
            if user.role != Role.technician:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST)
        await swe(users, shift, event_performance_number_identifier, redis)
...
async def all_workers(db: AsyncSession = Depends(get_db)):
    user = await get_workers(db)
    return user

@shift_supervisor_router.get("/all_events_done", response_model=List[EventSchema], dependencies=[Depends(shift_supervisor_only_access)], status_code=status.HTTP_200_OK)
async def all_events_done(shift: ShiftEnum, redis: Redis = Depends(get_redis)):
    events = await get_all_events_done(shift, redis)
    return events
